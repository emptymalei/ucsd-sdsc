...
subroutine GR_alp
  ...
  !Neutrino contribution to gravitational potential
  if (do_rotation) then
     nuE(:) = (4.0d0*W(:)**2*(v(:)**2+twothirds*vphi(:)**2)+1.0d0)*press_nu(:)
  else
     nuE(:) = (4.0d0*(W(:)*v(:))**2+1.0d0)*press_nu(:)
  endif
